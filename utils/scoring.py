...
def multiclass_dice_score(targets, predictions, num_classes):
    ...
    dice_scores[0] = np.nanmean(list(dice_scores.values()))
    return dice_scores
...
def simple_lesion_count_difference(truth, prediction, batchwise=False):
    ...
    '''
    if(not batchwise):
        _, pred_count = scipy.ndimage.label(prediction) # type: ignore
        _, truth_count = scipy.ndimage.label(truth) # type: ignore
        return np.abs(pred_count - truth_count)
    else:
        truth_shape = truth.shape
        count_list = []
        for i in range(truth_shape[0]):
            _, pred_count = scipy.ndimage.label(prediction[i, ...])     # type: ignore
            _, truth_count = scipy.ndimage.label(truth[i, ...])     # type: ignore
...
def lesion_count_by_weighted_assignment(truth, prediction, batchwise=False):
    ...
    for idx_sample in range(truth_shape[0]):
        ...
        pred_lesions, num_pred_lesions = scipy.ndimage.label(prediction[idx_sample, ...])   # type: ignore
        truth_lesion, num_truth_lesions = scipy.ndimage.label(truth[idx_sample, ...])   # type: ignore
...
def _lesion_f1_score(truth, prediction, empty_value=1.0):
    ...
    labeled_ground_truth, num_lesions = scipy.ndimage.label(truth.astype(bool)) # type: ignore
    ...
    labaled_prediction, num_pred_lesions = scipy.ndimage.label(prediction.astype(bool))  # type: ignore
